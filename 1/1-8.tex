...
\item [1.8.7.2]
  ...
  Because $g$ is monic, we know that $j; g = (k; f'); g$, therefore $j = (k; f')$.
  Furthermore, for any other object $X'$ with an arrow $j' : X' \rightarrow B$ we have the equalities $j'; g = (k'; f'); g$, $j' = k'; f'$, and $k'; f' = k; f'$.
  (We elide the arrow $i'$ from $X$ to the codomain of $g'$ to keep the diagram simpler.)
  Via this last equality, we see that $k' = k$.
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \node[above left of=1,xshift=-1cm,yshift=1cm] (7) {$X$};
      \draw[dashed,->] (7) -- node[above]{$k$} (1);
      \draw[->] (7) -- node[above]{$k; f'$} (2);
      \draw[->] (7) -- node[left]{$i$} (4);
      %% second pullback
      \node[above left of=2,xshift=2cm,yshift=1cm] (8) {$X'$};
      \draw[dashed,->] (8) -- node[above]{$k'$} (1);
      \draw[->] (8) -- node[above]{$j'$} (2);
      %% \draw[->] (8) -- node[left]{$i'$} (4);
    \end{tikzpicture}
  \end{center}
