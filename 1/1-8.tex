...
\item [1.8.7.2]
  ...
  From the above diagram, we know that $f'; g = j; g$.
  Therefore we have that $f' = j$; as a consequence, $X$ must be $P$ and $k$ must be $id_P$.
  \begin{center}
    \begin{tikzpicture}
      \node (1) {$P$};
      \node[right of=1,xshift=1cm] (2) {};
      ...
      \draw[->] (2) -- node[right] {$g$} (5);
      %% pullback stuff
      \node[above left of=1,xshift=-1cm,yshift=1cm] (7) {$P$};
      \draw[dashed,->] (7) -- node[above]{$id_A$} (1);
      \draw[->] (7) -- node[above]{$f'$} (2);
      \draw[->] (7) -- node[left]{$i$} (4);  
    \end{tikzpicture}
  \end{center}
  TODO
