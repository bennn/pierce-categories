...
\item[2.4.12.1] %% dualize the initial objects to get terminal objects
  Exercise \textbf{2.4.5} showed how an initial object in a category $\cat$ arises as the image of the unique object $*$ of the category $\one$ under a left adjoint to the constant functor $T : \cat \rightarrow \one$.
  
  Dually, a final object 1 in a category $\cat$ arises as the \emph{preimage} of the unique object $*$ of the category $\one$ under a \emph{right} adjoint to the constant functor $T$.
  The unit picks out the unique $\cat$-arrow from any $\cat$-object to the final object, 1, and the counit is fixed as $I_\one \rightarrow I_\one$.
  
  The unit and counit diagrams are below.
  The functor $G : \one \rightarrow \cat$ maps the object $*$ to the final object $1$ in $\cat$.
  \begin{center}
      \begin{tikzpicture}
        \node (1) {$*$};
  ...
  The existence and uniqueness of the arrow $f^{\#}$ follows because $1$ is a final object in $\cat$.
  Hence there exists a unique arrow for each $C \in \cat$ mapping $C$ to $1$.
...
\item[2.4.12.2]
  ...
  The unit of the adjunction carries a $\cat$-object to its product and the counit of the adjuction collapses the identity coproduct into a $\cat$-object.
  \begin{center}
      \begin{tikzpicture}
        \node (1) {$C$};
        \node [right of=1, xshift=2cm] (2) {$C \times C$};
...
\newpage
\item[2.4.12.3]
  Example \textbf{2.4.8} showed that $\eval_{AB}$ was the counit of the adjunction between the functors $F = (- \times A)$ and $G = (-)^A \times A$.
...
\subitem
  The unit of this adjunction is the class of arrows that take a fixed $\cat$-object $C$ to the pair of a $\cat$-object $A$ and $\cat$-arrows from $A$ to $C$.
